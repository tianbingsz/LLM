...
def compute_group_normalized_rewards(
    ...
    n_groups = N // group_size # batch_size


    # grouping: [n_groups = batch_size, group_size=G]
...
def sample_g_outputs(
    ...
    return all_sampled_outputs # (G * batch_size)
...
def reorder_samples_by_prompt(all_sampled_outputs, num_prompts, group_size):
    """
    Reorder the flat list of sampled outputs from (G * batch_size)
    [ sample1_prompt1, sample1_prompt2, ..., sample1_promptN,
      sample2_prompt1, sample2_prompt2, ..., sample2_promptN,
      ...
      sampleG_prompt1, sampleG_prompt2, ..., sampleG_promptN ]
    to (batch_size * G)
    [ prompt1_sample1, prompt1_sample2, ..., prompt1_sampleG,
      prompt2_sample1, prompt2_sample2, ..., prompt2_sampleG,
      ...
      promptN_sample1, promptN_sample2, ..., promptN_sampleG ]

    Args:
        all_sampled_outputs (list): flat list of all sampled outputs (length = num_prompts * group_size)
        num_prompts (int): number of distinct prompts
        group_size (int): number of samples per prompt (G)

    Returns:
        list: reordered list with samples grouped per prompt
    """
    reordered_outputs = []
    for i in range(num_prompts):
        for g in range(group_size):
            reordered_outputs.append(all_sampled_outputs[g * num_prompts + i])
...
def grpo_training_loop(
    ...
    for epoch in range(max_epochs):
        for step, batch in enumerate(dataloader):
            ...
            prompt_strs = batch["prompt_str"] # (batch_size)
            groundtruth_strs = batch["groundtruth_str"] # (batch_size)
            ...
            # Reorder sampled outputs so samples per prompt are contiguous (G * batch_size)
            num_prompts = len(prompt_strs)
            # (batch_size * G)
            correct_order_outputs = reorder_samples_by_prompt(all_sampled_outputs, num_prompts, G)

            # Step 3: Compute group-normalized advantages
            repeated_groundtruths = groundtruth_strs * G # (G * batch_size)
            # Reorder groundtruths same as outputs (to align with rewards), (batch_size * G)
            reordered_groundtruths = reorder_samples_by_prompt(repeated_groundtruths, num_prompts, G)

            # Compute group-normalized advantages (group_size = G samples per prompt)
            advantages, raw_rewards, stats = compute_group_normalized_rewards(
                reward_fn=reward_fn,
                rollout_responses=correct_order_outputs,
                repeated_ground_truths=reordered_groundtruths,
                group_size=G,
                advantage_eps=advantage_eps,
                normalize_by_std=normalize_by_std,
            )
            advantages_2d = advantages.view(num_prompts, G).to(device)  # shape (B, G)


            # Group outputs by G for training
            # This is the original order from sample_g_outputs, grouped by sampling iteration:
            grouped_outputs = [
                all_sampled_outputs[g * num_prompts:(g + 1) * num_prompts]
                for g in range(G)
            ] # (G, num_prompts or batch_size)

            # Step 4: Training steps per rollout batch corresponding to each batch of prompt_strs
            # g = 0, first batch of rollouts, g=1, second batch of rollouts for the same prompts
            for g in range(G):
                output_strs = grouped_outputs[g] # (num_prompts samples for iteration g)
                ...
                # batch_size = len(prompt_strs), Correctly index the slice of advantages corresponding to this sampling iteration
                group_advantages = advantages_2d[:,g].view(-1, 1)  # (batch_size, 1)

                # grpo micro batch train (backward)
...
grpo_training_loop(policy_model, dataloader, tokenizer, n_grpo_steps=3)